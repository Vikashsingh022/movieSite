...
const Home = () => {
  ...
  function getCategoryFromURL() {
    ...
    if (cat && [...Object.keys(categories), 'Favourites'].includes(cat)) {
      return cat;
    }
    return 'Popular';
  }
  ...
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const cat = params.get('category');
    if (cat && cat !== selectedCategory && [...Object.keys(categories), 'Favourites'].includes(cat)) {
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="w-full max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {displayData.length === 0 ? (
            <div className="col-span-full text-3xl text-gray-400 mt-10 text-center">No movies found.</div>
          ) : (
            displayData.map((obj) => (
